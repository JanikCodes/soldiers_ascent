...
namespace RenownedGames.ExLib.Maths
{
    public class CompositeShape
    {
        ...
        public void Process()
        {
            ...
            CompositeShapeData[] eligibleShapes = shapes.Select(s => new CompositeShapeData(s.GetPoints().Select(v => new Vector3(v.x, 0, v.y)).ToArray())).Where(c => c.IsValidShape).ToArray();
