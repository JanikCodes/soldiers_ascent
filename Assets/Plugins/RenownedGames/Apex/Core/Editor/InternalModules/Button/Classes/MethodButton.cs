...
namespace RenownedGames.ApexEditor
{
    public abstract class MethodButton : SerializedMember
    {
        ...
        public MethodButton(SerializedObject serializedObject, string memberName) : base(serializedObject, memberName) 
        {
            ...
            prefixLabel = prefixLabelAttribute != null && !string.IsNullOrEmpty(prefixLabelAttribute.label)
                ? new GUIContent(prefixLabelAttribute.label) : GUIContent.none;
        ...
        protected virtual float GetButtonHeight(float width, GUIStyle style)
        {
            return customHeight == -1 ? style.CalcHeight(GetLabel(), width) : customHeight;
        }
        ...
        /// </summary>
        protected sealed override float GetMemberHeight()
        {
            return GetButtonHeight(width, GetStyle());
        }
        #endregion
