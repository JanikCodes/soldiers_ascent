...
namespace RenownedGames.ApexEditor
{
    public sealed class ValueField<T> : VisualEntity, IValueField<T>
    {
        ...
        public ValueField(string name) : base(name) 
        {
            ...
            {
                helpBoxContent = new GUIContent($"ValueField does not support drawing custom types like {typeof(T).Name}. Implement new visual entity to draw your custom types.");
            }
        }

        public ValueField(string name, T value) : this(name)
        {
            this.value = value;
        }
        ...
        private void OnFieldGUI(Rect position, out object objValue)
        {
            ...
            switch (valueType)
            {
                ...
                case ValueType.Object:
                    objValue = EditorGUI.ObjectField(position, GetName(), (Object)objValue, typeof(T), true);
                    break;
