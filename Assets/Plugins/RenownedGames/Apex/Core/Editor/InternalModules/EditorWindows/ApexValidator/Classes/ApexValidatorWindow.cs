    ...
    sealed class ApexValidatorWindow : EditorWindow
    {
        ...
        private struct Entry : IComparable<Entry>
        {
            ...
            public Entry(Type target, MonoScript editorScript, bool subclasses)
            {
                ...
                content = new GUIContent($"File: <b>{editorScript.name}.cs</b>  <i>(Override: {target.Name}{(subclasses ? " (Including subclasses)" : string.Empty)})</i>");
        ...
        private void OnGUI()
        {
            ...
            GUI.Label(pos, "<b>Apex Validator</b> found that a custom editor was written for the following components. If you want to use the Apex editor for these components, click the <i>Fix</i> button.\n<i>This does not apply to other components, Apex will work without changes for them.</i>\nOtherwise, ignore this message and close the window.", messageStyle);
        ...
        //[MenuItem("Tools/Apex/Apex Validator", false, 15)]
        private static void Validate()
        {
            ...
            for (int i = 0; i < types.Count; i++)
            {
                ...
                if (inspectedType != null 
                    && (inspectedType == typeof(UnityEngine.Object)
