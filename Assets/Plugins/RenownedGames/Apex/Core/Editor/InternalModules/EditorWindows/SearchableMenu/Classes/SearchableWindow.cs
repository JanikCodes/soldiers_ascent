...
namespace RenownedGames.ApexEditor.Window.Searchable
{
    public sealed class SearchableWindow : EditorWindow
    {
        ...
        private void OnSearchFieldGUI()
        {
            ...
            if (GUI.Button(new Rect(toolbarPosition.xMax - 13, toolbarPosition.y - 4, 26, 23), EditorGUIUtility.IconContent("winbtn_win_close"), ApexStyles.BoxCenteredButton))
            {
                searchText = string.Empty;
        ...
        public void Sort(Comparison<SearchItem> comparison)
        {
        ...
        public void Sort()
        {
            items?.Sort(AlphabeticalComparison);
        }
        ...
        {
            return Mathf.Clamp(GetItemsHeight() + 22, 42, maxHeight);
        }

        #region [Static Methods]
        public static SearchableWindow Create()
        {
            return CreateInstance<SearchableWindow>();
        }
        #endregion

        #region [Comparison Implemetation]
        public Comparison<SearchItem> AlphabeticalComparison
        {
            get
            {
                return new Comparison<SearchItem>((s1, s2) => s1.GetLabel().text.CompareTo(s2.GetLabel().text));
            }
        }
        #endregion
        ...
        public bool FlexableHeight()
        {
            return flexableHeight;
        }

        public void FlexableHeight(bool value)
        {
            flexableHeight = value;
        }
        #endregion
    }
}