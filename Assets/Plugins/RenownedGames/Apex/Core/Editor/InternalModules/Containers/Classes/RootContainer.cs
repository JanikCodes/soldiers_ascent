...
namespace RenownedGames.ApexEditor
{
    public sealed class RootContainer : ListContainer, IRootContainer
    {
        ...
        internal RootContainer(string name, SerializedObject serializedObject, UnityAction repaint) : base(name)
        {
        ...
        {
            return GetEntitiesHeight(in entities);
        }
        #endregion
...
#pragma warning disable CS0618 // Type or member is obsolete
                ...
                if (smAttribute != null)
                {
                    try
                    {
                        SerializedMethod serializedMethod = new SerializedMethod(serializedObject, methodInfo.Name)
                        {
                            Repaint = repaint
                        };
                    ...
                    {
                        Debug.LogError($"Failed to create a serialized method {methodInfo.Name} of the {serializedObject.targetObject.GetType().Name} object. (Method has been ignored)");
                    }
                }
        ...
        internal static void FindProperties(in SerializedObject serializedObject, ref List<VisualEntity> visualEntities, ref int order, in UnityAction repaint)
        {
            ...
            foreach (PropertyInfo propertyInfo in type.AllProperties(limitDescendant))
            {
                ...
                if (attribute != null)
                {
                    ...
                    {
                        Debug.LogError($"Failed to create a serialized property {propertyInfo.Name} of the {serializedObject.targetObject.GetType().Name} object. (Property has been ignored)");
                    }
                }
            }
        }
            ...
            for (int i = 0; i < visualEntities.Count; i++)
            {
                ...
                if (visualElement is SerializedMember serializedMember)
                {
                    ...
                    if (attribute != null)
                    {
                        ...
                        tabGroup = new TabContainer(attribute.Name, new List<TabContainer.Tab>())
                        {
                            Repaint = repaint
                        };
                        tabGroup.AddEntity(attribute.Key, visualElement);
                ...
                IL_02:
                    for (int j = 0; j < boxGroup.GetEntityCount(); j++)
                    {
                        ...
                        if (child is SerializedMember childMember)
                        {
                            ...
                            if (attribute != null)
                            {
                                TabContainer tabGroup = boxGroup.GetEntities().Where(e => e is TabContainer tg && tg.GetName() == attribute.Name).FirstOrDefault() as TabContainer;
                                if (tabGroup != null)
                                {
        ...
        IL_01:
            for (int i = 0; i < visualEntities.Count; i++)
            {
                ...
                if (visualElement is SerializedMember serializedMember)
                {
                    ...
                    if (attribute != null)
                    {
                        FoldoutContainer foldout = visualEntities.Where(e => e is FoldoutContainer f && f.GetName() == attribute.Name).FirstOrDefault() as FoldoutContainer;
                        if (foldout != null)
                        {
                    ...
                    foreach (TabContainer.Tab pair in tabGroup.GetTabs())
                    {
                        for (int j = 0; j < pair.entities.Count; j++)
                        {
                            ...
                            if (child is SerializedMember childMember)
                            {
                                ...
                                if (attribute != null)
                                {
                                    FoldoutContainer foldout = tabGroup.GetTabs().Where(d => d.name == pair.name).SelectMany(m => m.entities).Where(e => e is FoldoutContainer f && f.GetName() == attribute.Name).FirstOrDefault() as FoldoutContainer;
                                    if (foldout != null)
                                    {
                ...
                IL_03:
                    for (int k = 0; k < boxGroup.GetEntityCount(); k++)
                    {
                        ...
                        if (child is SerializedMember childMember)
                        {
                            ...
                            if (attribute != null)
                            {
                                FoldoutContainer foldout = boxGroup.GetEntities().Where(e => e is FoldoutContainer f && f.GetName() == attribute.Name).FirstOrDefault() as FoldoutContainer;
                                if (foldout != null)
                                {
                                    foldout.AddEntity(child);
        ...
        IL_01:
            for (int i = 0; i < visualEntities.Count; i++)
            {
                ...
                if (visualElement is SerializedMember serializedMember)
                {
                    ...
                    if (attribute != null)
                    {
                        HorizontalContainer horizontalGroup = visualEntities.Where(e => e is HorizontalContainer h && h.GetName() == attribute.Name).FirstOrDefault() as HorizontalContainer;
                    ...
                    foreach (TabContainer.Tab pair in tabGroup.GetTabs())
                    {
                        for (int j = 0; j < pair.entities.Count; j++)
                        {
                            ...
                            if (child is SerializedMember childMember)
                            {
                                ...
                                if (attribute != null)
                                {
                                    HorizontalContainer horizontalGroup = tabGroup.GetTabs().Where(d => d.name == pair.name).SelectMany(m => m.entities).Where(e => e is HorizontalContainer h && h.GetName() == attribute.Name).FirstOrDefault() as HorizontalContainer;
                                    if (horizontalGroup != null)
                                    {
                            ...
                            IL_0201:
                                for (int k = 0; k < foldout.GetEntityCount(); k++)
                                {
                                    ...
                                    if (fChild is SerializedMember fChildMember)
                                    {
                                        ...
                                        if (attribute != null)
                                        {
                                            HorizontalContainer horizontalGroup = foldout.GetEntities().Where(e => e is HorizontalContainer h && h.GetName() == attribute.Name).FirstOrDefault() as HorizontalContainer;
                                            if (horizontalGroup != null)
                                            {
                ...
                IL_03:
                    for (int j = 0; j < boxGroup.GetEntityCount(); j++)
                    {
                        ...
                        if (bgChild is SerializedMember bgChildMember)
                        {
                            ...
                            if (attribute != null)
                            {
                                HorizontalContainer horizontalGroup = boxGroup.GetEntities().Where(e => e is HorizontalContainer h && h.GetName() == attribute.Name).FirstOrDefault() as HorizontalContainer;
                                if (horizontalGroup != null)
                                {
