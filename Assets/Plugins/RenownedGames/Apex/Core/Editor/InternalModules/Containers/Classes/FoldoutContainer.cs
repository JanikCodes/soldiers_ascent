...
namespace RenownedGames.ApexEditor
{
    public class FoldoutContainer : ListContainer, IFoldoutContainer, IGUIChangedCallback
    {
        ...
        public FoldoutContainer(string name, List<VisualEntity> entities) : base(name, entities)
        {
            headerHeight = 22;
        }
        ...
        /// <param name="entities">Foldout container entities.</param>
        public FoldoutContainer(string name, string style, List<VisualEntity> entities) : this(name, entities)
        {
            this.style = style;
        }
        ...
        protected virtual void OnHighlightGUI(Rect position)
        {
            ...
            {
                EditorStyles.foldoutHeader.Draw(position, GetName(), isHover, false, isExpanded, false);
            }
        ...
        protected virtual void OnHeaderGUI(Rect position)
        {
            ...
            {
                position.x += 4;
                ApexStyles.BoldFoldout.Draw(position, GetName(), isHover, false, isExpanded, false);
                position.x -= 4;
            }
        ...
        protected virtual float GetGroupHeight()
        {
            ...
            if (isExpanded)
            {
                height += (ApexGUIUtility.BoxBounds * 2)
                + base.GetHeight();
            }
            return height;
        }
