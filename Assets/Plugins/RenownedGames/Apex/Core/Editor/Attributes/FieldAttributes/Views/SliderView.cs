...
namespace RenownedGames.ApexEditor
{
    ...
    public sealed class SliderView : FieldView, ITypeValidationCallback
    {
        ...
        public override void OnGUI(Rect position, SerializedField serializedField, GUIContent label)
        {
            ...
            {
                case SerializedPropertyType.Integer:
                    serializedField.GetSerializedProperty().intValue = EditorGUI.IntSlider(position, label, serializedField.GetSerializedProperty().intValue, Convert.ToInt32(min), Convert.ToInt32(max));
                    break;
                case SerializedPropertyType.Float:
                    serializedField.GetSerializedProperty().floatValue = EditorGUI.Slider(position, label, serializedField.GetSerializedProperty().floatValue, min, max);
                    break;
            }
        }
